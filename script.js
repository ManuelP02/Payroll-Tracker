...
const displayAverageSalary = function(employeesArray) {
  ...
  const calculateAverageSalary = (employeesArray) =>{
    const totalSalary = employeesArray.reduce((acc, {salary}) => acc + salary, 0);
    const averageSalary = totalSalary / employeesArray.length;
    return averageSalary.toLocaleString("en-US", { style: "currency", currency: "USD" });
 }
...
const getRandomEmployee = function(employeesArray) {
  ...
  if (!Array.isArray(employeesArray) || employeesArray.length === 0) {
    console.error('Invalid employees array provided.');
    return;
  }
